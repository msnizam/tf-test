...
public class EntryResource {
	...
	public ResponseEntity<EntryDTO> createEntry(@Valid @RequestBody EntryDTO entryDTO) throws URISyntaxException {
		...
		Optional<BlogDTO> blogList = blogService.findOne(entryDTO.getBlogId());
		if (((entryDTO.getEmoji().equals(Emoji.LIKE) || entryDTO.getEmoji().equals(Emoji.HAHA))
				&& !blogList.get().isPositive())
				|| ((entryDTO.getEmoji().equals(Emoji.ANGRY) || entryDTO.getEmoji().equals(Emoji.SAD))
						&& blogList.get().isPositive())) {

			throw new BadRequestAlertException("Invalid Emoji", ENTITY_NAME, "invalidEmoji");
		}

		if (blogList.get().isPositive()) {
			if (entryDTO.getContent().contains("Angry") || entryDTO.getContent().contains("Sad")
					|| entryDTO.getContent().contains("Fear") || entryDTO.getContent().contains("Cry")
					|| entryDTO.getContent().contains("Lonely")) {
				throw new BadRequestAlertException("Invalid Content", ENTITY_NAME, "invalidContent");
			}
		}

		if (!blogList.get().isPositive()) {
			if (entryDTO.getContent().contains("Like") || entryDTO.getContent().contains("Love")
					|| entryDTO.getContent().contains("Happy") || entryDTO.getContent().contains("Haha")
					|| entryDTO.getContent().contains("Laugh")) {
				throw new BadRequestAlertException("Invalid Content", ENTITY_NAME, "invalidContent");
			}
		}
	...
	@DeleteMapping("/blogs/clean?keywords={values}")
	public ResponseEntity<Void> deleteBlogEntries(@PathVariable String values) {
		List<String> keywordList = Splitter.on("%2C").splitToList(values);
		log.debug("REST request to delete Blog : {}", keywordList);
		List<Entry> entryList = entryRepository.findAll();
		List<Entry> deleteEntryList = new ArrayList<>();

		for (String keyword : keywordList) {
			for (Entry entry : entryList) {
				if (!deleteEntryList.contains(entry)) {
		...
		if (!CollectionUtils.isEmpty(deleteEntryList)) {
			for (Entry entry : deleteEntryList) {
				entryService.delete(entry.getId());
			}
		}
		return ResponseEntity.noContent()
				.headers(HeaderUtil.createEntityDeletionAlert(applicationName, false, ENTITY_NAME, values)).build();
	}

	@DeleteMapping("/blogs/{id}/clean?keywords={values}")
	public ResponseEntity<Void> deleteBlogEntriesByID(@PathVariable Long id, @PathVariable String values) {
		List<String> keywordList = Splitter.on("%2C").splitToList(values);
		log.debug("REST request to delete Blog : {}", keywordList);
		...
		for (String keyword : keywordList) {
			for (Entry entry : entryList) {
				if (!deleteEntryList.contains(entry)) {
					if (entry.getBlog().getId().equals(id) && entry.getContent().contains(keyword)) {
						deleteEntryList.add(entry);
					}
				}
			}
